...
- **On-Screen Keyboard**: `BusinessSettings.keyboardMode` = `"off" | "builtin" | "windows-osk"` (set in Back Office → Printer Settings). `components/VirtualKeyboard.tsx` is mounted globally in `_layout.tsx`; on web it listens to `focusin`/`focusout` and either (a) shows a built-in floating keyboard with English/Arabic layouts (٤×11, Shift, Backspace, Space, Enter — uses native value setter + `input` event so React-controlled inputs update), or (b) calls `window.electronPOS.openOSK()` which spawns Windows `osk.exe` via `cmd.exe /c start "" osk.exe` from `desktop-installer/main.js` (`osk:open` / `osk:close` IPC). Keyboard polls business settings every 4 s so changes take effect without restart.
- **Test buttons**: Receipt Designer has "Test Print" (sample 105 AED receipt with current unsaved design+margins). KOT Settings has "Test KOT Print" (sample burger/fries/cola ticket to `windowsKOTPrinterName`, falls back to receipt printer). Both honour RAW/ESC-POS mode + auto-cut + codepage.
- **Hold modal**: `app/(tabs)/index.tsx` shows ALL tables (not just `available`/`reserved`) inside a scrollable grid with status badges; `occupied` tables are dimmed and disabled. Empty-state points the user to the Tables tab to add one.
