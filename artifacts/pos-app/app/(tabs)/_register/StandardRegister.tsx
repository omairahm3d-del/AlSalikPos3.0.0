...
export function StandardRegister() {
  ...
  const {
    ...
    loadStaff, loadAllModifierGroups,
  } = useDatabase();
  const { currentStaff } = useStaff();
  const { tableLabelSingular, tableLabel, productLabel } = useWorkMode();
  ...
  const fetchData = useCallback(async () => {
    const [prods, tbls, biz, tgs, cats, rdrs, staff, allGroups] = await Promise.all([
      loadProducts(), loadTables(), loadBusinessSettings(), loadTaxGroups(),
      loadCategories(), loadRiders(), loadStaff(), loadAllModifierGroups(),
    ]);
  ...
  }, [cartItems, heldOrderInfo, orderType, currentStaff, saveHeldOrder, clearCart, fetchData, kotSettings, businessSettings, session]);
  ...
  ), [colors, searchQuery, clearSearch, productLabel]);
    ...
    <View style={styles.cartInner}>
      ...
      {cartItems.length === 0 ? (
        <EmptyState
          icon="shopping-cart"
          title="Cart is empty"
          subtitle="Tap products or scan a barcode"
        />
      ) : (
        <FlatList
          data={cartItems}
